...
vim.keymap.set("n", "<space>M", "<cmd>source %<CR>")
vim.keymap.set("n", "<space>m", ":.lua<CR>")
vim.keymap.set("v", "<space>m", ":lua<CR>")
...
-- custom mappings https://github.com/jooize/vim-colemak
-- (c     )  Command-line mode
-- ( i    )  Insert mode
-- (  n   )  Normal mode
-- (   o  )  Operator pending
...
if os.getenv("COLEMAK") == "true" then
  -- Colemak setup
  -- better up/down
  map({ "n", "o", "x" }, "n", "v:count == 0 ? 'gj' : 'j'", { expr = true, silent = true, desc = "Colemak down" })
  map({ "n", "o", "x" }, "e", "v:count == 0 ? 'gk' : 'k'", { expr = true, silent = true, desc = "Colemak up" })
  map({ "n", "o", "x" }, "m", "h", { desc = "Colemak left" })
  map({ "n", "o", "x" }, "i", "l", { desc = "Colemak right" })

  map({ "n", "o", "x" }, "l", "e", { desc = "Colemak end" })
  map({ "n", "x" }, "j", "m", { desc = "Colemak mark/set" })
  map({ "n" }, "h", "i", { desc = "Colemak insert" })
  map({ "o", "x" }, "h", "i", { desc = "Colemak inside" })
